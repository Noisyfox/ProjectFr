...
class Test_1_Login(unittest.TestCase):
    ...
    def testNormalPwd(self):
        for uid,name,pwd,sex,ty,avatar,sch,reg,sid in config.users:
            p = {
                'method': 'user.login',
                'name': name,
                'password': pwd
...
class Test_2_Modify(unittest.TestCase):
    ...
    def te_stNormal_withoutpwd(self):
        for uid, name, pwd, sex, ty, avatar, school, region, sid in random.sample(config.users, 3):
    ...
    def testInvalidSession_nothing(self):
        for uid,name,pwd,sex,ty,avatar,sch,reg,sid in random.sample(config.users, 3):
            p = {
                'method': 'user.modify',
                'uid': 999,
                'session': '=,=',
